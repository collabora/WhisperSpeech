...
# %% auto 0
__all__ = ['load_datasets', 'DelSumEmbedding', 'DelSumHeadLayer', 'rand', 'Tunables', 'SADelARTransformer', 'make_model']
...
class DelSumEmbedding(nn.Module):
    def __init__(self, width, quantizers, codes=1024, length=2250):
        super().__init__()
        self.codes = codes
        self.width = width
    ...
    def forward(self, toks, xenc, use_cache=True, offset=0):
        if use_cache:
            ...
            for i in range(self.quantizers):
                ...
                if i < n and i < self.quantizers - 1:
...
class DelSumHeadLayer(nn.Module):
    def __init__(self, width, quantizers, codes=1024):
        super().__init__()
        self.width = width
        self.codes = codes
        self.quantizers = quantizers
    ...
    def forward(self, x, newn):
        logits = self.heads(x).view(x.shape[0], newn, self.quantizers, self.codes + 1).permute(0, 2, 1, 3)
        return logits

# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT.ipynb 20
import dataclasses
import random
...
# %% ../nbs/3C. Multi-speaker semantic to acoustic token modeling MusicGen μPT.ipynb 21
