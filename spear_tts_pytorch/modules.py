# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/B. Modules.ipynb.

# %% auto 0
__all__ = ['sinusoids', 'MultiHeadAttention', 'ResidualAttentionBlock', 'Encoder', 'Decoder', 'SumDecoder']

# %% ../nbs/B. Modules.ipynb 2
import torch
import numpy as np
import math
...
# %% ../nbs/B. Modules.ipynb 9
class SumDecoder(nn.Module):
    def __init__(self, depth=6, width=384, n_head=6, length=9000, codes=1024, pos_embs=None):
        super().__init__()
        self.length = length
        self.codes = codes
        ...
        self.layers = nn.ModuleList([
            ResidualAttentionBlock(width, n_head) for _ in range(math.floor(depth/2))
        ])

        # after adding the encoder features
        self.layers2 = nn.ModuleList([
            ResidualAttentionBlock(width, n_head) for _ in range(math.ceil(depth/2))
        ])
