...
# %% auto 0
__all__ = ['load_datasets', 'rand', 'Tunables', 'Encoder', 'T2SEmbeddingWithPositional', 'TSARTransformer', 'make_model']
...
class T2SEmbeddingWithPositional(nn.Module):
    ...
    def forward(self, Stoks, xenc_dtype, offset=0):
...
class TSARTransformer(nn.Module):
    ...
    def forward(self, Ttoks, Stoks, loss=True, offset=0):
        with record_function("encoder"):
            xenc = self.encoder(Ttoks.to(torch.long))
        with record_function("decoder"):
            x = self.embedding_layer(Stoks, xenc.dtype, offset=offset)
    
            x = self.decoder(x, xenc)
